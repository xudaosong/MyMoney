(function () {
    ...
    VoiceBroadcastController.$inject = ['$scope', 'Restangular', '$modal', 'dialog', '$templateCache'];

    /* @ngInject */
    function VoiceBroadcastController($scope, Restangular, $modal, dialog, $templateCache) {
        ...
        function parse() {
            ...
            angular.forEach(rows, function (row) {
                ...
                if (!!Date.parse(text)) {
                    ...
                    item.created = year + (angular.element(row).text().replace('·', '').trim());
                } else {
        ...
        function batchSave() {
            voiceBroadcast.post(vm.parseData).then(function (res) {
                console.log(res);
            });
        }
    }
